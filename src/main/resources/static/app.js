...
async function api(path, options = {}) {
  const response = await fetch('/api' + path, {...options,headers:{'Content-Type':'application/json',...(csrf?{[csrf.headerName]:csrf.token}:{}),...options.headers}});
  if(response.status===401){location.assign('/login.html');throw new Error('Please sign in again.');}
...
async function ensureSession() {
  if (!session) { session = (await api('/conversations', {method:'POST'})).id; localStorage.setItem(sessionKey,session); }
  return session;
}
...
async function send(text) {
  ...
  try {
    ...
    const response = await fetch(`/api/conversations/${session}/messages`, {method:'POST', headers:{'Content-Type':'application/json','Accept':'text/event-stream',[csrf.headerName]:csrf.token},body:JSON.stringify({message:text})});
...
$('#new-session').addEventListener('click',()=>{if(!busy){localStorage.removeItem(sessionKey);location.reload();}});
...
async function loadSessions(){
  ...
  for(const item of sessions){
    ...
    b.addEventListener('click',async()=>{if(busy)return;try{session=item.id;localStorage.setItem(sessionKey,session);await restore();}catch(e){notice(e.message);}});$('#session-list').append(b);
  }
}
...
$('#logout').addEventListener('click',async()=>{
  ...
  try{const response=await fetch('/logout',{method:'POST',headers:{[csrf.headerName]:csrf.token}});if(!response.ok)throw new Error('Sign-out failed. Please refresh.');location.assign('/login.html');}catch(error){notice(error.message);}
});
$('#download-excel').addEventListener('click',async()=>{
  try{const response=await fetch('/api/admin/excel');if(!response.ok)throw new Error(response.status===404?'The ledger is not ready. A safety report must complete its Excel step first.':'Download failed.');const url=URL.createObjectURL(await response.blob());const a=document.createElement('a');a.href=url;a.download='mindbridge-reports.xlsx';a.click();setTimeout(()=>URL.revokeObjectURL(url),1000);}catch(e){notice(e.message);}
});
(async()=>{
  try{
    csrf=await api('/csrf');profile=await api('/profile');
    sessionKey='mindbridge-session-'+profile.username;session=localStorage.getItem(sessionKey);
    ...
    else {try{await restore();}catch(e){session=null;localStorage.removeItem(sessionKey);notice(e.message);}await loadSessions();}
